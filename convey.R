...
for(i in 1:length(taglist)) {
  ...
  conveyLaTex(dat,paste("relation/driver/multiRegExplo_",taglist[i],"_latexcode.txt",sep = ""))
}

for(i in 1:length(taglist)) {
  dat <- read.csv(paste("relation/driver/multiRegRefined_",taglist[i],".csv",sep = ""))
  dat <- dat[,-6:-8]
  dat <- dplyr::transmute_(dat, "parameter" = "parameter",
                           "Estimate" = "signif(Estimate,3)",
                           "Std..Error" = "signif(Std..Error,3)",
  ...
  colnames(dat)[c(5,7)] <- c("p(Estimate)","p(ANOVA)")
  dat$parameter <- as.character(dat$parameter)
  conveyLaTex(dat,paste("relation/driver/multiRegRefined_",taglist[i],"_latexcode.txt",sep = ""))
}